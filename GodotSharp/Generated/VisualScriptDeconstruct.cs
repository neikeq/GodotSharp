using System;

namespace GodotEngine
{
    public class VisualScriptDeconstruct : VisualScriptNode
    {
        private const string nativeName = "VisualScriptDeconstruct";

        public VisualScriptDeconstruct() : this(true)
        {
            if (ptr == IntPtr.Zero)
                ptr = NativeCalls.godot_icall_VisualScriptDeconstruct_Ctor(this);
        }

        internal VisualScriptDeconstruct(bool memoryOwn) : base(memoryOwn) {}

        ~VisualScriptDeconstruct()
        {
            Dispose();
        }

        public override void Dispose()
        {
            lock (this)
            {
                if (ptr != IntPtr.Zero)
                {
                    if (memoryOwn)
                    {
                        memoryOwn = false;
                    NativeCalls.godot_icall_VisualScriptDeconstruct_Dtor(ptr);
                        ptr = IntPtr.Zero;
                    }
                }

                GC.SuppressFinalize(this);
            }
        }

        private IntPtr method_bind_0 = NativeCalls.godot_icall_TypeDB_get_method(nativeName, "set_deconstruct_type");

        public void set_deconstruct_type(int type)
        {
            NativeCalls.godot_icall_1_4(method_bind_0, Object.GetPtr(this), type);
        }

        private IntPtr method_bind_1 = NativeCalls.godot_icall_TypeDB_get_method(nativeName, "get_deconstruct_type");

        public int get_deconstruct_type()
        {
            return NativeCalls.godot_icall_0_0(method_bind_1, Object.GetPtr(this));
        }

        private IntPtr method_bind_2 = NativeCalls.godot_icall_TypeDB_get_method(nativeName, "set_deconstruct_input_type");

        public void set_deconstruct_input_type(int input_type)
        {
            NativeCalls.godot_icall_1_4(method_bind_2, Object.GetPtr(this), input_type);
        }

        private IntPtr method_bind_3 = NativeCalls.godot_icall_TypeDB_get_method(nativeName, "get_deconstruct_input_type");

        public int get_deconstruct_input_type()
        {
            return NativeCalls.godot_icall_0_0(method_bind_3, Object.GetPtr(this));
        }
    }
}
